...
def get_projection_files(proj_type, data_name):
    """Returns the path to all files generated by a given projection type."""
    main_dir = path.join(get_main_dir(data_name), 'projections', data_name)
    proj_dir = path.join(main_dir, proj_type)
    makedirs(proj_dir, exist_ok=True)
    files = {'root': main_dir,
             'meshes_similarity': path.join(proj_dir, 'meshes_similarity.npy'),
...
class Projection:
    ...
    def __init__(self, proj_type, data_name, facedata, model, load_matrices=False):
        ...
        files = get_projection_files(proj_type, data_name)
